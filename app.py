...
def format_sources_html(search_results, news_results):
    ...
    for r in (news_results or [])[:3]:
        sources.append({'title':r.get('title',''),'url':r.get('url',''),'source':r.get('source',''),'type':'news'})
...
def init_session_state():
    defaults = {'messages':[],'current_view':'analysis','scan_running':False,
                ...
                'chart_symbol':'FX:EURUSD','chart_interval':'D','last_scan_check':0}
...
FOREX_PAIRS = {"EUR/USD":"EURUSD=X","GBP/USD":"GBPUSD=X","USD/JPY":"USDJPY=X",
...
CRYPTO_PAIRS = {"BTC/USD":"BTC-USD","ETH/USD":"ETH-USD","SOL/USD":"SOL-USD",
    "XRP/USD":"XRP-USD","BNB/USD":"BNB-USD","ADA/USD":"ADA-USD",
    "DOGE/USD":"DOGE-USD","DOT/USD":"DOT-USD","AVAX/USD":"AVAX-USD"}
STOCKS = {"Apple":"AAPL","Tesla":"TSLA","NVIDIA":"NVDA","Google":"GOOGL",
    "Amazon":"AMZN","Microsoft":"MSFT","Meta":"META","Netflix":"NFLX",
    "AMD":"AMD","Intel":"INTC"}
TIMEFRAMES = {"15 دقيقة":{"interval":"15m","period":"5d"},
...
TRACKING_INTERVALS = {"15 دقيقة":{"interval":"5m","period":"5d"},
    "1 ساعة":{"interval":"15m","period":"1mo"},
    "4 ساعات":{"interval":"1h","period":"1mo"},
    "يومي":{"interval":"1h","period":"3mo"}}
TV_SYMBOLS = {
    "فوركس":{"EUR/USD":"FX:EURUSD","GBP/USD":"FX:GBPUSD","USD/JPY":"FX:USDJPY",
        "USD/CHF":"FX:USDCHF","AUD/USD":"FX:AUDUSD","NZD/USD":"FX:NZDUSD",
        "USD/CAD":"FX:USDCAD","EUR/GBP":"FX:EURGBP","EUR/JPY":"FX:EURJPY","GBP/JPY":"FX:GBPJPY"},
    "سلع":{"الذهب":"COMEX:GC1!","الفضة":"COMEX:SI1!","النفط":"NYMEX:CL1!",
        "الغاز":"NYMEX:NG1!","النحاس":"COMEX:HG1!"},
    "عملات رقمية":{"BTC/USD":"CRYPTO:BTCUSD","ETH/USD":"CRYPTO:ETHUSD",
        "SOL/USD":"CRYPTO:SOLUSD","XRP/USD":"CRYPTO:XRPUSD","BNB/USD":"CRYPTO:BNBUSD",
        "ADA/USD":"CRYPTO:ADAUSD","DOGE/USD":"CRYPTO:DOGEUSD","AVAX/USD":"CRYPTO:AVAXUSD"},
    "أسهم أمريكية":{"Apple":"NASDAQ:AAPL","Tesla":"NASDAQ:TSLA","NVIDIA":"NASDAQ:NVDA",
        "Google":"NASDAQ:GOOGL","Amazon":"NASDAQ:AMZN","Microsoft":"NASDAQ:MSFT",
        "Meta":"NASDAQ:META","Netflix":"NASDAQ:NFLX","AMD":"NASDAQ:AMD","Intel":"NASDAQ:INTC"},
    "مؤشرات":{"S&P 500":"FOREXCOM:SPXUSD","Nasdaq":"NASDAQ:NDX","Dow Jones":"DJ:DJI",
        "DAX":"XETR:DAX","FTSE 100":"FOREXCOM:UKXGBP"}
}
...
def get_ai_final_decision(ai_client, ticker, name, tech, fund, news, filters, price, hint):
    ...
    try:
        resp = ai_client.chat_completion(
            messages=[{"role":"system","content":"خبير تداول. JSON فقط."},
...
def build_filters_text(tech_filters,tech_score,fund_details,fund_score,news_details,news_score,ai_score,ai_reasoning,ai_confidence):
...
def full_analysis(ticker, name, tf_key, ai_client, require_strong=False):
    ...
    return {'ticker':ticker,'name':name,'price':price,'direction':direction,'signal':sig_label,
        ...
        'ai_risk':ai_risk,'tech_filters':tech_filters,'fund_details':fund_details,'news_details':news_details,
        'filters_text':filters_text,'targets':tgts,'timeframe':tf_key,'curr':curr}

def smart_update_signal(sig_row):
    ticker=sig_row['ticker']; entry=float(sig_row.get('entry_price',0) or 0)
    tp1=float(sig_row.get('tp1',0) or 0); tp2=float(sig_row.get('tp2',0) or 0)
    sl=float(sig_row.get('sl',0) or 0); is_buy=sig_row.get('direction','buy')=='buy'
    ...
    for name,tick in assets_dict.items():
        ...
        try:
            ...
            if result and result['price']>0:
                added=db.add_signal(str(tick),str(name),result['direction'],float(result['price']),
                    float(result['targets']['tp1']),float(result['targets']['tp2']),float(result['targets']['tp3']),
                    float(result['targets']['sl']),float(abs(result['total_score'])),str(scan_tf),
...
required=['init_db','add_signal','get_active_signals','get_closed_signals',
    'update_signal_status','save_analysis','set_scan_status','get_scan_status','delete_all_active']
