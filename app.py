...
def calculate_indicators(df):
    ...
    try:
        stoch = ta.momentum.StochasticOscillator(h, l_col, c)
    ...
    try:
        df['ATR'] = ta.volatility.average_true_range(h, l_col, c)
    except Exception:
        pass

    try:
        adx_ind = ta.trend.ADXIndicator(h, l_col, c)
        df['ADX'] = adx_ind.adx()
    ...
    try:
        df['PSAR'] = ta.trend.PSARIndicator(h, l_col, c).psar()
    except Exception:
        pass

    try:
        ich = ta.trend.IchimokuIndicator(h, l_col)
    ...
    try:
        resp = ai_client.chat_completion(
            messages=[
                {
                    "role": "system",
                    "content": "أنت محلل مالي محترف. أجب فقط بتنسيق JSON صالح. لا تكتب أي كود برمجي. فقط JSON."
                },
...
if st.session_state.current_view == "signals":
    ...
    with btn_col2:
        ...
        try:
            ...
            st.error(f"خطأ في قراءة التوصيات: {db_err}")
            signals_list = []
        ...
        if signals_list and len(signals_list) > 0:
            for sig_row in signals_list:
                try:
                    ...
                    sig_progress = float(sig_row.get('progress', 0) or 0)
                    sig_pnl = float(sig_row.get('pnl_pct', 0) or 0)
                    sig_current = float(
                        sig_row.get('current_price', 0)
                        or sig_row.get('entry_price', 0)
                        or 0
                    )
                    sig_entry = float(sig_row.get('entry_price', 0) or 0)
                    sig_tp2 = float(sig_row.get('tp2', 0) or 0)
                    sig_sl = float(sig_row.get('sl', 0) or 0)
                    sig_name = sig_row.get('asset_name', '')
                    sig_ticker = sig_row.get('ticker', '')

                    st.markdown(f"""
                    <div class="rec-card" style="border-left: 5px solid {color};">
                        <div style="display:flex; justify-content:space-between;">
                            <h3 style="margin:0;">{sig_name}
                        ...
                        <div style="font-size:14px; margin:8px 0; display:flex; justify-content:space-between;">
                            <span>🏁 الدخول: {sig_entry:.4f}</span>
                            <span>🏷️ الحالي: {sig_current:.4f}</span>
                            <span>🎯 الهدف: {sig_tp2:.4f}</span>
                            <span>🛑 الوقف: {sig_sl:.4f}</span>
                        </div>
        ...
        if closed_list and len(closed_list) > 0:
            hist_data = []
            for closed_row in closed_list:
                try:
                    hist_data.append({
                        "التاريخ": closed_row.get('timestamp', ''),
                        "الأصل": closed_row.get('asset_name', ''),
                        "الاتجاه": "شراء" if closed_row.get('direction') == 'buy' else "بيع",
                        "الحالة": "✅ هدف" if closed_row.get('status') == 'tp_hit' else "❌ وقف",
                        "الربح %": round(float(closed_row.get('pnl_pct', 0) or 0), 2)
                    })
                except Exception:
                    continue
            if hist_data:
    ...
    if analyze_btn:
        with st.spinner("جاري جلب البيانات وتحليل السوق..."):
            ...
            if df is not None and len(df) > 15:
                ...
                try:
                    db.save_analysis(
                        ticker, tf_label, sig, cls_name, comb,
    ...
    if 'analysis_result' in st.session_state:
        ...
        with tab3:
            ...
            if ai_res and isinstance(ai_res, dict):
                st.info(
                    f"🎯 القرار: **{ai_res.get('final_decision', 'غير محدد')}**"
                )
                st.write(
                    f"📝 التحليل: {ai_res.get('reasoning', 'غير متوفر')}"
                )
...
القواعد الإلزامية:
1. أجب باللغة العربية فقط مهما كانت لغة السؤال.
2. ممنوع منعاً باتاً كتابة أي كود برمجي بأي لغة برمجة.
3. ممنوع استخدام علامات الكود مثل ``` أو ` نهائياً.
4. إذا قال لك أحد hi أو hello أو أي تحية بالإنجليزية، رحب به بالعربية وعرف نفسك كمستشار مالي متخصص.
5. تخصصك: التحليل الفني والأساسي، إدارة المخاطر، استراتيجيات التداول، الأسواق المالية.
6. كن مختصراً وعملياً.
7. حذر دائماً من مخاطر التداول."""
                    ...
                    st.error(
                        "⚠️ حدث خطأ في الاتصال بالذكاء الاصطناعي. "
                        "حاول مرة أخرى."
                    )
            else:
                st.error(
